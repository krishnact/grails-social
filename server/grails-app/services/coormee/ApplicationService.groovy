...
class ApplicationService {
    DefaultAuthorityStorageService authStorageService
...
//    }
    ...
    OAuth2SpringToken createAccount(OAuth2SpringToken oAuth2SpringToken, String userName, String password) {
        OAuth2CreateAccountCommand command = new OAuth2CreateAccountCommand();
        command.username = userName
        command.password1= password
        def user = null
        User.withTransaction {TransactionStatus status->
            user = authStorageService.createUserRolesAndAuths(command, oAuth2SpringToken, status)
        }
        if (user){
            authStorageService.updateOAuthToken(oAuth2SpringToken, user)
